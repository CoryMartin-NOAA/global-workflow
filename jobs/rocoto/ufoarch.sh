...
# create ARCHDIR
mkdir -p $ARCHDIR/$CDATE/$CDUMP.$PDY/$cyc/

# move output from ROTDIR to ARCHDIR
mv -f $ROTDIR/$CDUMP.$PDY/$cyc/* $ARCHDIR/$CDATE/$CDUMP.$PDY/$cyc/.

# link bias corr files
mkdir -p $ARCHDIR/$CDATE/$CDUMP.$fPDY/$fcyc/atmos
ln -sf $ICSDIR/$FDATE/$CDUMP.$PDY/$cyc/*abias* $ARCHDIR/$CDATE/$CDUMP.$PDY/$cyc/atmos/.
